...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO booking (
            group_id, tour_name, booking_date, tour_date, 
            first_name, last_name, lang, number_of_tourists,
            tour_type, voucher_no, agent, pickup_time, 
            hotel, room_no, phone_no, phone_no_extra, 
            whatsapp, telegram, viber, wechat, email, 
            country, city, adult_no, child_no, 
            infant_no, payment_method, price, paid_cash, 
            paid_card, vehicle, food, tickets, 
            note, agent_id, vehicle_id, net_price, 
            category, adult_tickets, adult_food, child_tickets, child_food
        ) VALUES (
            :group_id, :tour_name, :booking_date, :tour_date, 
            :first_name, :last_name, :lang, :number_of_tourists,
            :tour_type, :voucher_no, :agent, :pickup_time, 
        ...
        $stmt->execute([
            ...
            ':last_name' => $lastName,
            ':lang' => $lang,
            ':number_of_tourists' => $adultNo + $childNo + $infantNo,
            ':tour_type' => $tourType,
            ':agent' => '',
            ...
            ':note' => $note,
            ':agent_id' => $agentId,
            ':vehicle_id' => null, // Or set appropriately
            ':net_price' => $price - ($paidCash + $paidCard), // Adjust as needed
        ...
        echo "Error duplicating booking: " . $e->getMessage();
        exit();
    }
}
?>
...
<div class="wrapper-fit-centered">
    <h2 class="my-20 text-center">Duplicate Booking</h2>
    <form method="post" action="" class="booking">
        <div>
            ...
            <script>
                ...
                function generateVoucherNumber() {
                    ...
                    xhr.open('GET', 'voucher.php', true);
                    xhr.onreadystatechange = function() {
                        if (xhr.readyState == 4 && xhr.status == 200) {
                            var response = JSON.parse(xhr.responseText);
                            if (response && response.voucher_number) {
                                document.getElementById('voucher_no').value = response.voucher_number;
                            }
                        }
                    };
                    xhr.send();
                }
                // Call the function to generate voucher number when the page loads
            ...
            <label for="available_tours_search">Available Tours:</label>
                <input type="text" id="available_tours_search" placeholder="Type available tour name..." autocomplete="off">
        ...
        <div>
            ...
            <label for="adult_food">Adult Food:</label>
            ...
            <label for="child_food">Child Food:</label>
...
$(document).ready(function() {
    ...
    $(document).on('click', '.result-item', function() {
        ...
        $('#agent_search').val(selectedAgent); 
